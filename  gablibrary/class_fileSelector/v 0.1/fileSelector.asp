...
'' @CDESCRIPTION:	Lets you easily browse files in a given folder and select any of them.
'' @VERSION:		0.1
...
class fileSelector
	...
	public property let selected(val) ''[array] array with full virtual-paths of the files which should be selected. If multipleSelection is off then the first item is taken
		for i = 0 to uBound(val)
			selectedFiles.add val(i), server.mapPath(val(i))
		next
	end property
	
	public property get selected ''[array] gets the selected files. usefull after postback
		if request.form.count > 0 then
			selected = split(request.form("FSSelectedFiles"), ", ")
		else
			selectedArray = array()
	...
	private sub drawTree(folder, depth)
		for each subFolder in folder.subfolders
			...
			for each field in selectedFiles.items
				'represents the str.startsWith function.
				if uCase(left(field, len(subFolder))) = subFolderUCased then
	...
	private sub printFolder(folder, folderID, opened, depth)
		...
		print("<div class=folder onclick=""FSToggleFolder('" & folderID & "',this)"">")
	...
	private sub printFile(file, depth)
		...
		fileLocation = virtualSourcePath & mid(replace(file.path, "\", "/"), len(sourcePath) + 2, len(file.path))
		...
		if filesSelectable then
			...
			if fileSelected and not multipleSelection then
				print("<script>FSSelectedFileID='lb" & fileID & "'</script>")
			end if
		end if
