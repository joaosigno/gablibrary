...
class Spinner
	...
	public firstInstance	'' [bool] OBSOLETE! determines if this instance is the first on the page.
	...
	public enableMouseWheel  ''[bool] allows you to change the spinner using the mouse wheel (default = true)
	...
	public sub draw(spin_ID, startValue)
		includeScripts spin_ID, startValue
		drawSpinnerControl spin_ID, startValue
	end sub
	...
	private sub includeScripts(spinID, startValue)
		...
		if skin = SKIN_TWO then
			lib.page.loadStylesheetFile classLocation & "styles/two.css", empty
			lib.page.loadJavascriptFile(classLocation & "styles/two.js")
		else
			lib.page.loadStylesheetFile classLocation & "styles/default.css", empty
			lib.page.loadJavascriptFile(classLocation & "styles/default.js")
		end if
	...
	private sub drawSpinnerTextfield(spinID, startValue)
		...
		if enableMouseWheel then
			onMouseWheel = "return shiftUpDown(this, '" & step & "', " & minimum & ", " & maximum & ", '" & looping & "');" & onchange
		else
			onMouseWheel = empty
		end if
		...
		writeln("<input type=""text"" id=""" & spinID & """ name=""" & spinID & """ " & checkReadonly() & " autocomplete=""off"" value=""" & startValue & """ " &_
