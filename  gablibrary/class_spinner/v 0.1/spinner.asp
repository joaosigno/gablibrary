...
class spinner
	...
	private sub includeScripts(spinID, startValue)
		...
		if firstInstance then
			...
			if skin = SKIN_TWO then
				str.writeln("<link rel=stylesheet type=text/css href=""" & classLocation & "styles/two.css"">")
				str.writeln("<script language=""JavaScript"" src=""" & classLocation & "styles/two.js""></script>")
			else
				str.writeln("<link rel=stylesheet type=text/css href=""" & classLocation & "styles/default.css"">")
				str.writeln("<script language=""JavaScript"" src=""" & classLocation & "styles/default.js""></script>")
			end if
			firstInstance = false
		end if
		...
		str.writeln("var vdo" & spinID & " = " & orientation & ";")
		str.writeln("var vdd" & spinID & " = " & decimalPlaces & ";")
		str.writeln("var vdi" & spinID & " = " & integerPlaces & ";")
		
		if IsArray(controlItems) and controltype = CONTROL_CUSTOM then
			str.writeln("var vdCustomMode_" & spinID & " = 1;")
			response.write("var vdCustomValues_" & spinID & " = new Array(")
			...
			next
			str.writeln(tmp)
			str.writeln("var vdArrayCount_" & spinID & " = " & pStartID & ";")
		else
			str.writeln("var vdCustomMode_" & spinID & " = 0;")
		end if
	...
	private sub drawSpinnerTextfield(spinID, startValue)
		...
		onBlur	= "checkCorrectInput(this, " & minimum & ", " & maximum & ");" & onchange
		
		str.writeln("<input type=""text"" id=""" & spinID & """ name=""" & spinID & """ " & checkReadonly() & " value=""" & startValue & """ " &_
	...
	private sub drawIncreaseButton(spinID)
		...
		str.writeln("<button class=""spinnerControlButton " & cssClass & """ id=""up_" & spinID & """ onclick=""" & onClick & """ tabindex=""-1"">" &_
	...
	private sub drawDecreaseButton(spinID)
		...
		str.writeln("<button class=""spinnerControlButton " & cssClass & """ id=""down_" & spinID & """ onclick=""" & onClick & """ tabindex=""-1"">" &_
	...
	private sub drawSpinnerControl(spinID, startValue)
		...
		str.writeln("<span style=""margin-right:" & BUTTON_WIDTH & "px;width:" & recalculatedWidth & "px;" & styleAttribute & """>")
		if orientation = ORIENTATION_TOPDOWN then
			call drawSpinnerTextfield(spinID, startValue)
			str.writeln("<span class=spinnerControlButtons>")
			drawIncreaseButton(spinID)
			drawDecreaseButton(spinID)
			str.writeln("</span>")
		else
			str.writeln("<span class=spinnerControlButtons>")
			drawDecreaseButton(spinID)
