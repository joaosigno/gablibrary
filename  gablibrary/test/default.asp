...
function drawFiles(folder, caption, ajaxedFolder)
	if lcase(folder.name) = "gab_library" and not ajaxedFolder then exit function
	for each file in folder.files
		if str.startsWith(file.name, "test_") and str.endsWith(file.name, ".asp") then
			if ajaxedFolder then
				path = "/gab_library" & unmapPath(file.path, "/gab_library/")
			else
	...
	<div style="float:left;width:30%">
		<div style="height:300px;overflow:auto;padding:5px;" id="tests">
			...
			<div id="ajaxedTests">
				...
				<% drawFiles lib.fso.getFolder(server.mappath("/gab_library/")), link, true %>
			</div>
			<br>
			<div id="otherTests">
				<strong>
