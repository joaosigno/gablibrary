...
function goToDate(urlToLoad, selectedDate, calendarUrl) {
	...
	openCenteredModal(calendarUrl + "&selectedDate=" + selectedDate, 300, 280, false);
...
function showDayMenu(dayviewUrl, currentDate, weekDayName) {
	menu = document.getElementById("dayMenu");
	menuHeader = document.getElementById("dayMenuItemHeadline");
	
	if (arguments.length > 1) {
		document.getElementById("dayMenuClickedDate").value = currentDate;
		menuHeader.innerText = weekDayName + ", " + currentDate;
	}
	...
	var mouseX = window.event.clientX + window.document.body.scrollLeft - 10;
	var mouseY = window.event.clientY + window.document.body.scrollTop - 10;
	var bottomSpace = document.body.clientHeight - event.clientY - 10;
	var rightSpace = document.body.clientWidth - event.clientX - 10;
