...
var m_val_max;
var m_looping;
var imagePath = "/gab_Library/class_spinner/";

function preloadImages() {
	var preImg = new Array(8);
	for(i = 0; i < m_Images.length; i++) {
		preImg[i] = new Image();
		preImg[i].src = m_Images[i];
	}
}
...
{	
	checkCorrectInput(obj, val_min, val_max, step);
...
function checkCorrectInput(obj, val_min, val_max, step)
{
	var tmp = parseFloat(obj.value);
	if(isNaN(tmp))
		if(eval("vdCustomMode_" + obj.id + " != 1"))
	...
	if(!((val_min == 0) && (val_max == 0)))
	{
		...
		if(digits > 1) {
			if(step > 1) {
				if(obj.value % step < (step/2))
					obj.value = val_min + Math.floor(obj.value/step)*step;
				else
					obj.value = val_min + Math.ceil(obj.value/step)*step;
			}
			obj.value = fillUp(obj.value, digits);
		}
	}
}
