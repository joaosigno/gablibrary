...
class Drawtable
	...
	private fltrField_				
	private p_fastDelete			
	private p_fastDeleteMessage		
	private xlsExporter				''[ExcelExporter] 
	...
	public fieldlinkID 				''[string] name of the primary key-column of the table.
	...
	public addurl					''[string] the url you want to redirect to when the user adds or edits a record.
	public addurlJS					''[string] if isEditable is true you can set a javascript call instead of an url
	public isSortable				''[bool] enable/disable sorting. default = true
	public showFilterBar			''[bool] enable/disable filterbar
	public headersPerRow			''[int] every how many rows do you want to display the headers again
	...
	public defaultStylesheet		''[bool] you need a default stylesheet? default = true
	...
	'' @RETURN:			[array] array with IDs (fieldLinkID) and value separated by ':' which have been changed.
	'**************************************************************************************************************
	...
	'******************************************************************************************************************
	'' @SDESCRIPTION:	Depreciated!!! use fastDelete-property instead
	'' @SDESCRIPTION:	should fast delete be enabled?
	...
	'**************************************************************************************************************
	'' @SDESCRIPTION:	Depreciated!!! Add field method. This is an old one. You should use addColumn-method
	...
	private sub  tableHeader()
		...
		if printing then
			printButton = "&nbsp;<button onclick=""window.print();"" class=button>" & printTxt & "</button>"
	...
	private sub showHeader(myFields, commonSort)
		...
		for each col in allTableColumns.items
			...
			addToHeaderVariable("<td class=head " & col.headerTdAttributes & " nowrap>" & myAhref & "<nobr><span class=currentSort>" & showCurrentSort & "</span></nobr>" & selectAllString & "</td>")
	...
	private sub filterBar(myFieldArr)
		...
		if not allFilters.count = 0 AND showFilterBar then 'we dont show anything if there are no filters defined
			...
			addToOutput("		<input type=submit name=go class=button_common style='width:0px;height:0px;'><span class=noprint><button title=""" & TOOLTIP_TXT_FILTERRESET & """ style='height:12px;cursor:hand;' onclick=""restoreMyFilter();myVeryHiddenForm.submit();"" name=restore_my_filter class=button_common>^</button></span>")
			addToOutput("</td>")
			addToOutput("</tr>")
		end if
	end sub
	...
	private sub genTable()
					...
					for each fil in allFilters.items
						if cstr(myFieldName) = cstr(fil.fieldName) then
							...
							if uBound(operatorArray) > -1 then
								splittedSQL(filterConditionPosition) = " (" & matchField & ") " & operatorArray(0) & " ('" & operatorArray(1) & "') AND " & splittedSQL(filterConditionPosition)
								...
								if fil.showDropdownSQL = empty then
									splittedSQL(filterConditionPosition) = " " & sqlCommands.Item("ucase") & "(" & matchField & ") LIKE " & sqlCommands.Item("ucase") & "('%" & searchKeyword & "%') AND " & splittedSQL(filterConditionPosition)
								else
									splittedSQL(filterConditionPosition) = " (" & matchField & " = '" & searchKeyword & "') AND " & splittedSQL(filterConditionPosition)
								end if
							end if
						end if
					next
					...
					if not cstr(fil.defaultSelect) = empty and request.form.count = 0 and tableSession.getFilterValue(fltrField_ & fil.fieldName) = empty then
						...
						if fil.showDropdownSQL = empty then
							...
							if uBound(operatorArray) > -1 then
								...
								splittedSQL(filterConditionPosition) = " UPPER(" & matchField & ") LIKE UPPER('%" & fil.defaultSelect & "%') AND " & splittedSQL(filterConditionPosition)
							end if
						else
							splittedSQL(filterConditionPosition) = " " & matchField & " = '" & fil.defaultSelect & "' AND " & splittedSQL(filterConditionPosition)
						end if
					end if
				next
			end if
		...
		'check the paging
		if paging then
			if timedebuging then startRecordSet = timer()
			...
			while not RS.eof
				if (paging and counter < recsPerPage) or not paging or showAllRecs then
					...
					with aRow
						...
						.ID = "tr_" & .recordID
						.BGColor = lib.iif(.index mod 2 = 0, ROW_COLOR_1, ROW_COLOR_2)
					end with
					...
					if aRow.CSSClass <> empty then TRString = TRString & " class=""" & aRow.CSSClass & """"
					...
					if fastDelete then
						...
						addToOutput("<button" & lib.iif(aRow.disabled, " disable", empty) & " title=""" & TOOLTIP_TXT_FASTDELETE & """ name=XXXYYYZZZ onclick=""yesNoFastDelete(" & currentRecordID & ", '" & fastDeleteMessage & TXTFASTDELETEQUESTION & "')"" class=button_common>")
		...
		set RS = nothing
		tableFooter counter, myLink, pageCount, sumRecs
		...
		for each col in allTableColumns.items
			...
			if col.colType = "radiobutton" then
				...
				if col.displayFunction <> "" then
					...
					if not isNull(fieldContent) then inputVar = replace(fieldContent, """", """""")
