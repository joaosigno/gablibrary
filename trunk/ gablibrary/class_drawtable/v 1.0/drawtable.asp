...
class drawtable
	...
	private sub Class_Initialize()
		...
		version					= "Drawtable class - ver 1.0"
	...
	private sub showHeader(myFields, commonSort)
		...
		for each col in allTableColumns.items
			...
			addToHeaderVariable("<TD class='head' " & col.headerTdAttributes & " nowrap><label title=""" & TOOLTIP_TXT_SORTBY & " " & col.displayString & """>" & myAhref & col.displayString & "</a></label><nobr><span class=currentSort>" & showCurrentSort & "</span></nobr>" & selectAllString & "</TD>" & vbcrlf)
	...
	private sub genTable(sqlQuery,fieldLinkId,addUrl,commonSort)
		...
		mysort = tableSession.getSortValue(allFilters, isSortable)  'get the sortValue to know what column to sort
		'sortieren beim Adressbuch
				...
				if (paging AND counter < recsPerPage) OR not paging OR showAllRecs then
					...
					if allowFastDelete OR showFilterBar then
						if allowFastDelete then
								...
								addToOutput (",'" & fastDeleteMessage & TXTFASTDELETEQUESTION & "')"" class=button_common>" & TXTFASTDELETEBUTTON & "</button></label>")
								addToOutput ("</span></TD>" & vbcrlf)
	...
	private sub displayFullSearchField()
		...
		addToOutput ("	<TD valign=middle width=100% >&nbsp;<LABEL title=""" & TOOLTIP_TXT_SEARCHALL & """><input class=formField type=text name=fullsearchtext value=""" & tableSession.getSearchValue() & """ " & onkeyPress & " style=""width:100px;""></LABEL></TD>" & vbcrlf)
	...
	private sub showTableData(myDebugID)
		for each col in allTableColumns.items
			if col.colType = "radiobutton" then
				...
				'addToOutput("	<TD align=center " & col.tdAttributes & " ><input type=""radio"" name=rb_" & objRsFieldlinkId & " id=rb_" & objRsFieldlinkId & " value=" & col.value & checked & " onclick=javascript:changeBG(rb_" & objRsFieldlinkId & "," & objRsFieldlinkId & ");" & "></TD>" & vbcrlf)
				addToOutput("	<TD align=center " & col.tdAttributes & " ><input type=""radio"" name=rb_" & objRsFieldlinkId & " value=" & col.value & checked & " onclick=""javascript:changeBG('" & col.value & "', " & objRsFieldlinkId & ");""></TD>" & vbcrlf)
			else
