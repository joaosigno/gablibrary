...
class drawtable
	...
	private sub Class_Initialize()
		version					= "Drawtable class - ver 0.98"
			...
			for each col in allTableColumns.items
				...
				for each fil in allFilters.items
					if col.fieldName = fil.fieldName then
						...
						myVal 		= request.form("fltrField_" & matched)
	...
	private sub genTable(sqlQuery,fieldLinkId,addUrl,commonSort)
		...
		mysort = request.form("sortValue") 'get the sortValue to know what column to sort
		...
		if showFilterBar then
			...
			for each fild in request.form
				myFieldName = empty
				if instr(fild,"fltrField_") and not request.form(fild) = empty and not request.form(fild) = "XXXYYYZZZXXX" then
