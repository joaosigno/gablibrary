...
class Drawtable
	...
	private fltrField_				
	private p_fastDeleteMessage		
	private xlsExporter				
	
	public RS						''Recordset Object	
	public allTableColumns			''Collection where your columns are stored. it can be usefull to access it on runtime
	...
	public fieldlinkID 				''name of the primary key-column of the table. e.g. used for edit-form.
	...
	public addurlJS					''if isEditable is true you can set a javascript call instead of an url
	...
	public sub Class_Initialize()
		...
		call tableSession.InitializeSessionObject
		
		version					= "Drawtable class - ver 1.0.7"
	...
	'******************************************************************************************************************
	public function set_allowFastDelete(val)
		if val then
	...
	private function deleteRecord()
		if allowFastDelete and autoDelete then
			delid = empty
	...
	private sub  tableHeader()
		...
		if not title = empty then
			addToOutput ("	<td class=headline>" & title & "</td>")
		end if
		addToOutput ("	<td align=right>")
		addToOutput ("		<span class=noprint>")
		addToOutput ("		<table border=0 cellpadding=0 cellspacing=0>")
		addToOutput ("		<tr>")
	...
	private sub showHeader(myFields, commonSort)
		...
		for each col in allTableColumns.items
			...
			if isSortable then
				myAhref = "<a class='head' href=""javascript:document.myVeryHiddenForm.sortValue.value='" & sortby & "';submitTableData();"">"
			end if
			
			addToHeaderVariable("<td class=head " & col.headerTdAttributes & " nowrap><label title=""" & TOOLTIP_TXT_SORTBY & " " & col.displayString & """>" & myAhref & col.displayString & "</a></label><nobr><span class=currentSort>" & showCurrentSort & "</span></nobr>" & selectAllString & "</td>")
	...
	private sub filterBar(myFieldArr)
		...
		if not allFilters.count = 0 AND showFilterBar then 'we dont show anything if there are no filters defined
			...
			for each col in allTableColumns.items
				...
				if not matched = "no" AND not lastFilter = matched then
					...
					if sql = "" then 'we make a common text filter field
						...
						addToOutput ("<label title=""" & tooltipText & """><input type=text value='" & myVal & "' class=formField name=" & fltrField_ & matched & " " & onChangeSubmit() & " ></label>")
			...
			addToOutput("		<input type=submit name=go class=button_common style='width:0px;height:0px;'><span class=noprint><label title=""" & TOOLTIP_TXT_FILTERRESET & """><button style='height:12px;cursor:hand;' onclick=""javascript:restoreMyFilter();myVeryHiddenForm.submit();"" name=restore_my_filter class=button_common>^</button></label></span>")
			addToOutput("</td>")
			addToOutput("</tr>")
		end if
	end sub
	...
	private function getAddLink(addUrl)
		...
		getAddlink = "<button class=button onclick=""" & myTarget & "location.href='" & addUrl & "'"">" & addTxt & "</button>"
	end function
	...
	private sub genTable()
		...
		if not addUrl = "" and allowAdding then myLink = getAddLink(addUrl)
					...
					for each fil in allFilters.items
						if cstr(myFieldName) = cstr(fil.fieldName) then
							...
							if uBound(operatorArray) > -1 then
								splittedSQL(1) = " (" & matchField & ") " & operatorArray(0) & " ('" & operatorArray(1) & "') AND " & splittedSQL(1)
		...
		addToOutput("<table width=" & tablewidth & " cellspacing=1 cellpadding=" & tableCellpadding & " border=0 class=""tableClass"">")
		...
		if allowFastDelete then
			...
			addToOutput(hiddenFieldString("fastDeleteID", empty))
		end if
		...
		if not RS.eof then
			...
			set currentRecordID = RS(fieldlinkid) 'our primary-key in an object. should be faster
			...
			while not RS.eof
				if (paging AND counter < recsPerPage) OR not paging OR showAllRecs then
					...
					addToOutput("<tr " & mouseover & " id=tr_" & currentRecordID & ">")
					'we display the id of the field if debuging is on
					if debuging then
						myDebugID = "(id: " & currentRecordID & ")&nbsp;"
					end if
					...
					if allowFastDelete OR showFilterBar then
						if allowFastDelete then
								addToOutput("<td align='right' valign=middle width=15><span class=noprint>")
								addToOutput("<label title=""" & TOOLTIP_TXT_FASTDELETE & """>")
								addToOutput("<button name='XXXYYYZZZ' onclick=""javascript:yesNoFastDelete(" & currentRecordID & ",'" & fastDeleteMessage & TXTFASTDELETEQUESTION & "')"" class=button_common>")
								addToOutput(TXTFASTDELETEBUTTON & "</button></label>")
								addToOutput("</span></td>")
						elseif showFilterBar then
							addToOutput("<td width=15></td>")
						end if
					end if
					addToOutput("</tr>")
					counter = counter + 1
	...
	private sub pagingBar(pageCount,counter)
		...
		if not absolutePage = 1 and not counter = 0 and not showAllRecs then
			addToOutput("<label title=""" & TOOLTIP_TXT_PREVPAGE & """><a href=""javascript:document.myVeryHiddenForm.actualPageNumber.value='" & absolutePage - 1 & "';submitTableData();"" class=pagingPrevNext>" & TXTPAGINGPREV & "</a></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
		end if
		
		'show me the all link
		if not showAllRecs then
			addToOutput("<label title=""" & TOOLTIP_TXT_SHOWALL & """><a href=""javascript:document.myVeryHiddenForm.actualPageNumber.value='0';submitTableData();"" class=pagingLink>" & TXTPAGINGALL & "</a></label>")
		else
		...
		if not cint(absolutePage) = cint(pageCount) and not counter = 0 and not showAllRecs then
			addToOutput("&nbsp;&nbsp;&nbsp;&nbsp;<label title=""" & TOOLTIP_TXT_NEXTPAGE & """><a href=""javascript:document.myVeryHiddenForm.actualPageNumber.value='" & absolutePage + 1 & "';submitTableData();"" class=pagingPrevNext>" & TXTPAGINGNEXT & "</a></label>")
		end if
	end sub
	...
	private sub tableFooter(counter,myLink,pageCount,sumRecs)
		...
		if counter = 0 then
			...
			addToOutput("<tr><td align=center colspan=30><div class=noRecsFound>" & TXTNORECSAVAILABLE & "</div></td></tr>")
		end if
		addToOutput("</table>")
	...
	private sub displayFullSearchField()
		...
		addToOutput("	<td valign=middle width=100% >&nbsp;<input title=""" & TOOLTIP_TXT_SEARCHALL & """ id=fullsearchtext class=formField type=text name=fullsearchtext value=""" & tableSession.getSearchValue() & """ " & onkeyPress & " style=""width:100px;""></td>")
		addToOutput("	<td valign=middle nowrap>&nbsp;<input type=button name=button class=button value=search" & mySubmit & "></td>")
		addToOutput("</tr>")
		addToOutput("</table>")
	...
	private sub displayExcelExport()
		if excelexport then
			addToOutput("<td>&nbsp;<button onClick=""document.forms['xlform'].submit();"" class='button' name='exportto'>" & excelTxt & "</button></td>")
		end if
	end sub
	...
	private sub createExcelForm()
		if excelexport then
			with xlsExporter
				...
				addToOutput("<form name=xlform style=""diplay:inline"" method=post action=""/gab_Library/class_excelexporter/index.asp""" & excelFormTarget & ">")
	...
	private sub showTableData(myDebugID)
		for each col in allTableColumns.items
			if col.colType = "radiobutton" then
				...
				addToOutput("	<td align=center " & col.tdAttributes & " ><input type=""radio"" name=rb_" & currentRecordID & " value=" & col.value & checked & " onclick=""javascript:changeBG('" & col.value & "', " & currentRecordID & ");""" & lib.iif(col.disabled, " disabled", empty) & "></td>")
			else
				...
				if isEditable and not col.disableLink then
					if addUrlJS <> empty then
						tdRowJavascript = " onclick=""" & addUrlJS & """ style='cursor:hand;'"
					else
						...
						tdRowJavascript = " onclick=""" & myTarget & "location.href='" & addUrl & qsPrefix & fieldLinkid & "=" & currentRecordID & "'"" style='cursor:hand;'"
					end if
				...
				addToOutput("	<td " & col.tdAttributes & tdRowJavascript & ">" & myDebugID & editUrlBegin & field0 & editUrlEnd & "</td>")
