...
class Drawtable
	...
	public property get height ''[int] gets the height
		height = p_height
	end property
	...
	public sub Class_Initialize()
		...
		set legend				= nothing
		p_height				= 0
		cssLocation				= lib.iif(DRAWTABLE_CSS_LOCATION = "", DT_CLASSLOCATION & "standard.css", DRAWTABLE_CSS_LOCATION)
		showFooter 				= true
	...
	private sub tableHeader()
		...
		printButton = empty
		if printing then printButton = "&nbsp;<button onclick=""window.print();return false;"" type=button class=button>" & printTxt & "</button>"
		...
		if showTitleBar then
			...
			'add button
			if (not addUrl = "" or formURL <> "") and allowAdding then
				myLink = getAddLink()
	...
	private sub genTable()
		...
		if (not addUrl = "" or formURL <> "") and allowAdding then myLink = getAddLink()
	...
	private sub displayExcelExport()
		if excelexport then
			addToOutput("<td>&nbsp;<button type=button onClick=""document.forms['xlform'].submit();"" class='button' name='exportto'>" & excelTxt & "</button></td>")
		end if
	end sub
			...
			if col.colType = "radiobutton" then
				...
				if isEditable and not col.disableLink and not rowObject.disabled then
					if addUrlJS <> empty then
						tdRowJavascript = " onclick=""" & addUrlJS & """ style='cursor:pointer;'"
					else
						...
						if formURL <> "" then
							...
							myURL = addUrl & qsPrefix & fieldLinkid & "=" & currentRecordID
						end if
