...
'' @REQUIRES:		ErrorHandler, TextTemplate
'' @VERSION:		0.3
...
class Webservice
	...
	public disableWebserviceHostCheck ''[bool] disable/enable the isWebserviceHost check. default = false
									  ''implemented on the 21st Feb. 07 to prevent IE from alerting a cross site scripting warning
									  ''(because the webservice url is a different then the application url)
	...
	public sub class_initialize()
		...
		myBase.isXML = true
		errorHandling = true
		disableWebserviceHostCheck = false
	end sub
	...
	public sub generate()
		with myBase
			...
			elseif isWebserviceHost() and err = 0 then
				.drawContent()
	...
	private sub addError(errCode, errMessage)
		...
		if errCode <> ERRCODE_WRONGHOST and errCode <> ERRCODE_MAINTENANCE then
	...
	private function isWebserviceHost()
		...
		if disableWebserviceHostCheck then isWebserviceHost = true
	end function
